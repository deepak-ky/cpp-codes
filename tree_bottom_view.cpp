...
void insert_bottom_element_in_map(node * root, int h, int hd, map<int, pair<int, int> > &mp)
{
	if (root == NULL) return;
	...
	insert_bottom_element_in_map(root->left, h + 1, hd - 1, mp);
	insert_bottom_element_in_map(root->right, h + 1, hd + 1, mp);
}
void print_bottom_view(node * root)
{
	map<int, pair<int, int> > mp;
	insert_bottom_element_in_map(root, 0, 0, mp);
	for (auto x : mp)
	{
