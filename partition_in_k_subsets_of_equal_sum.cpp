...
void  partition_in_k_subsets(int idx, int *a, int n, int k, unordered_map<int, vector<int>> mp, int req_sum)
{
	if (idx == n)
	{

		bool not_matching_conditions = false;
		for (int i = 0; i < k; i++)
		{
			int c_sum = 0;
			for (auto x : mp[i])
	...
	for (int i = 0; i < k; i++)
	{
		if (mp[i].size() > 0)
		{
			...
			partition_in_k_subsets(idx + 1, a, n, k, mp, req_sum);
			mp[i].pop_back();
...
int32_t main()
{
	...
	while (t--)
	{
		...
		else
		{
			...
			partition_in_k_subsets(0, a, n, k, mp, sum / k);
		}
