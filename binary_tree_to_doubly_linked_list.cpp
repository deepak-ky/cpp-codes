...
void binary_tree_to_doubly_linked_list(node * root)
{
	if (root == NULL) return;
	static node * prev = NULL;
	binary_tree_to_doubly_linked_list(root->left);
	if (prev == NULL)
	{
		head = root;
	} else
	...
	prev = root;
	binary_tree_to_doubly_linked_list(root->right);
...
#endif
	node * root = buildtree();
	binary_tree_to_doubly_linked_list(root);
	printnode(head);
