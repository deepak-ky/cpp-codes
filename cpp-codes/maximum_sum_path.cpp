...
int maximumsumpath(node * root, int& global_max)
{
	if (root == NULL) return 0;
	int left = maximumsumpath(root->left, global_max);
	int right = maximumsumpath(root->right, global_max);
	int w = 0;
	int w1 = (max(w, max(left, right)) + root->data);
	int w2 = max(w1, (left + right + root->data));
	global_max = max(global_max, w2);
	return w1;
}
int findmaxsum(node * root)
{
	int global_max = INT_MIN;
	maximumsumpath(root, global_max);
	return global_max;
}
int32_t main()
{
...
#endif
	node * root = buildtree();
	cout << "\nthe maximum sum path is : " << findmaxsum(root);
