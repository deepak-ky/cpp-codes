...
void print_kth_level_opt(node * root, int k, int m)
{
	if (root == NULL)
	{
		return;
	...
	if (m & 1)
	{
		print_kth_level_opt(root->left, k - 1, m);
		print_kth_level_opt(root->right, k - 1, m);
	}
	else
	{
...
bool first_element_seen(node * root, int k)
{
	if (root == NULL)
	{
		return false;
	...
	if (!first_element_seen(root->left, k - 1))
	{
		first_element_seen(root->right, k - 1);
	}
}
void left_view_of_binary_tree(node * root)
{
	int h =  height_of_tree(root);
	for (int i = 1; i <= h; i++)
	{
		cout << first_element_seen(root, i) << endl;
	}
}
int32_t main()
{
