...
int range_sum_query(int *segment_tree, int s, int e, int idx, int qs, int qe)
{
	if (qs <= s && qe >= e)
	{
	...
	int mid = s + ((e - s) / 2);
	int op1 = range_sum_query(segment_tree, s, mid, 2 * idx, qs, qe);
	int op2 = range_sum_query(segment_tree, mid + 1, e, 2 * idx + 1, qs, qe);
	return op1 + op2;
}
...
int32_t main()
{
	...
	while (t--)
	{
		...
		int qs, qe;
		cin >> qs >> qe;
		cout << range_sum_query(segment_tree, 0, n - 1, 1, qs, qe);
